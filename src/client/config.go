...
func (cc *Connector) ReadConfig(data string) {
	...
	jsonObj.ForEach(func(key,value gjson.Result) bool {
		...
		}else if key.String() == "ControllerContainer" {
			config["ControllerContainer"] = value.String()
		}else {
	...
	keys := []string{"DataVolume","ReferVolume","ScriptUrl","StartRunCmd","AuthFile","GlusterEndpoints","Namespace","OutputBaseDir","ControllerContainer"}
