...
type Config struct {
	...
	SendMessageToController string  // 各个deployment向controller发送消息的通道
	ReceiveMessageFromController  string  // 各个deployment从controller接收消息的通道
...
func  NewK8sClient(redisAddr string) *K8sClient {
    ...
    contrCon,_ := db.RedisHashGet("kubernetesConfig","ControllerContainer")
    dataDir := "/mnt/data"
    referDir := "/mnt/refer"
	...
	td := &Config{
		...
		SendMessageToController: "Send-Message-To-Controller",
		ReceiveMessageFromController: "Receive-Message-From-Controller",
		RedisAddr: redisAddr,
		ScriptUrl: scriptUrl,
		ControllerContainer: contrCon,
		NameSpace: namespace,
		DataDir: dataDir,
					...
					Containers: []apiv1.Container{
						{
							...
							Env: []apiv1.EnvVar{
								...
								{
									Name: "LISTENCLIENT",
									Value: myutils.GetSamplePrefix(sample) + "-Client-To-Controller",
								},				
								{
									Name: "SENDTOCLIENT",
									Value:  myutils.GetSamplePrefix(sample) + "-Listen-From-Controller",
								},				
								{
									Name: "DATADIR",
